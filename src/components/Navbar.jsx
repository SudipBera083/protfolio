...
export default function Navbar() {
  ...
  return (
    ...
    >
      <Container>
        <div className={`relative mt-6 rounded-2xl backdrop-blur-xl border transition-all duration-500 ${
          scrolled
          ...
          <div className="flex items-center justify-between px-6 py-4">
            ...
            <Link to="/" className="flex items-center gap-3 group">
              ...
              <div className="hidden sm:block leading-tight">
                <div className="text-white font-semibold text-sm">Sudip Bera</div>
            ...
            <div className="flex items-center gap-4">
              ...
              >
                <motion.span animate={isOpen ? { rotate: 45, y: 6 } : {}} className="w-5 h-[2px] bg-white" />
                <motion.span animate={isOpen ? { opacity: 0 } : {}} className="w-5 h-[2px] bg-white" />
                <motion.span animate={isOpen ? { rotate: -45, y: -6 } : {}} className="w-5 h-[2px] bg-white" />
              </button>
            </div>
          </div>
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -10 }}
              className="md:hidden mt-3 rounded-2xl bg-black/80 backdrop-blur-xl border border-white/10 p-6 space-y-3"
            >
              {navLinks.map((link) => (
                <Link
                  key={link.path}
